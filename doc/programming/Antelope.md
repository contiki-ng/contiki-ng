...
values than the buffer holds fails with a limit error. `MEAN` uses integer
division and truncates toward zero. `SUM` and `MEAN` accumulate their sum in a
C `long`, so a large sum can overflow on platforms where `long` is 32 bits.
...
`WHERE` clause must also appear in the projection of the same `SELECT`, unless
the projection consists only of aggregate expressions. Examples:
...
or once `db_query()` or `db_process()` has returned an error. A plain `SELECT`
streams matching tuples to the caller without materializing them; only a query
whose result is assigned to a relation with `<-` writes a new relation to
storage.
...
projected unless the projection consists only of aggregate expressions, and a
projection cannot mix aggregate expressions with plain attributes.

### Schema changes

Attributes cannot be removed once they have been created. `REMOVE ATTRIBUTE` is
parsed but always fails with `DB_IMPLEMENTATION_ERROR`.
...
Join support and tuple removal are compile-time options (`DB_FEATURE_JOIN` and
`DB_FEATURE_REMOVE`) that are enabled by default. `DB_FEATURE_FLOATS`, which is
disabled by default, only makes the lexer accept floating-point literals; there
is no floating-point attribute domain.
