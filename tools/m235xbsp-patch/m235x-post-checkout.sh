...
#    [ -e ${STDDRIVER}/timer.c ] && echo ${STDDRIVER}/timer.c
    ...
    [ -e ${SYSTEM_M2351_c} -a "grep -q PRIx32 ${SYSTEM_M2351_c}" ] && patch -p1 < ${toplevel}/tools/m235xbsp-patch/system_M2351.patch
    SYSTEM_M2354_c="${library_dir}/Device/Nuvoton/M2354/Source/system_M2354.c"
    [ -e ${SYSTEM_M2354_c} -a "grep -q PRIx32 ${SYSTEM_M2354_c}" ] && patch -p1 < ${toplevel}/tools/m235xbsp-patch/system_M2354.patch
#    echo -n ""
fi
}

if [ -e "arch/cpu/m2351/bsp/Library" ]; then
    name="arch/cpu/m2351/bsp"
    cd ${toplevel}/${name} > /dev/null
    patch_m235x
fi

cd ${toplevel}
if [ -e "arch/cpu/m2354/bsp/Library" ]; then
    name="arch/cpu/m2354/bsp"
    cd ${toplevel}/${name} > /dev/null
    patch_m235x
fi
