...
<simconf version="2023090101">
  <simulation>
    <title>EDHOC simulation (METHOD 3, RFC 7529 ephemeral keys)</title>
    ...
    <motetype>
      ...
      <source>[CONTIKI_DIR]/tests/21-security-protocols/edhoc-client/edhoc-test-client.c</source>
      <commands>$(MAKE) TARGET=cooja clean
$(MAKE) -j$(CPUS) DEBUG=1 MAKE_EDHOC_METHOD=3 edhoc-test-client.cooja DEFINES=EDHOC_CONF_TEST=EDHOC_TEST_VECTOR_TRACE_DH TARGET=cooja</commands>
    ...
    <motetype>
      ...
      <source>[CONTIKI_DIR]/tests/21-security-protocols/edhoc-server/edhoc-test-server.c</source>
      <commands>$(MAKE) TARGET=cooja clean
$(MAKE) -j$(CPUS) DEBUG=1 MAKE_EDHOC_METHOD=3 edhoc-test-server.cooja DEFINES=EDHOC_CONF_TEST=EDHOC_TEST_VECTOR_TRACE_DH TARGET=cooja</commands>
      <moteinterface>org.contikios.cooja.interfaces.Position</moteinterface>
      <moteinterface>org.contikios.cooja.interfaces.Battery</moteinterface>
      <moteinterface>org.contikios.cooja.contikimote.interfaces.ContikiVib</moteinterface>
      <moteinterface>org.contikios.cooja.contikimote.interfaces.ContikiMoteID</moteinterface>
...
while (true) {
  ...
  if (msg.contains("Client finished")) {
    if (hasError) {
       log.testFailed();
    } else if (checkingEnabled &amp;&amp; correctCount == 0) {
       // Only fail on zero correctCount if checking was enabled
       log.testFailed();
    } else {
       log.testOK();
    }
  ...
  if(msg.contains("Using test vector: false") || msg.contains("Test Vector Mode: Production") || 
     msg.contains("Connection method: 0") || msg.contains("Method: 0") || 
     msg.contains("Connection method: 1") || msg.contains("Method: 1") || 
     msg.contains("Connection method: 2") || msg.contains("Method: 2")) {
  ...
  // Check if info for SALT_4e3m is correct (only check label 5 operations)
  if (msg.contains("INFO for HKDF_Expand (37 bytes): 05 58 20")) {
    if (msg.contains("INFO for HKDF_Expand (37 bytes): 05 58 20 ad af 67 a7 8a 4b cc 91 e0 18 f8 88 27 62 a7 22 00 0b 25 07 03 9d f0 bc 1b bf 0c 16 1b b3 15 5c 18 20")) {
  ...
  if (msg.contains("SALT_4e3m")) {
    if (!msg.contains("INFO for HKDF_Expand")) {
  ...
  // Only check actual TH_4 value, not headers like "Computing TH_4" or "TH_3 for TH_4"
  if (msg.contains("TH_4 (32 bytes):") &amp;&amp; !msg.contains("for TH_4")) {
  ...
  if (msg.contains("RX message_3 CBOR (") || msg.contains("TX message_3 CBOR (")) {
    if (msg.contains("RX message_3 CBOR (19 bytes): 52 e5 62 09 7b c4 17 dd 59 19 48 5a c7 89 1f fd 90 a9 fc") || msg.contains("RX message_3 CBOR (20 bytes): 27 52 e5 62 09 7b c4 17 dd 59 19 48 5a c7 89 1f fd 90 a9 fc") || msg.contains("TX message_3 CBOR (20 bytes): 27 52 e5 62 09 7b c4 17 dd 59 19 48 5a c7 89 1f fd 90 a9 fc")) {
